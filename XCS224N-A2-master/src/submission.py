...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  difference = y_hat
  difference[outside_word_idx] -= 1
  grad_center_vec = outside_vectors.T @ difference
  grad_outside_vecs = np.outer(difference, center_word_vec)
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for word in outside_words:
    outside_word_index = word2ind[word]
    additional_loss, additional_grad_center_vector, additional_grad_outside_vector = word2vec_loss_and_gradient(center_word_vector, outside_word_index, outside_vectors, dataset)
    loss += additional_loss
    grad_center_vecs[center_word_index] += additional_grad_center_vector
    grad_outside_vectors += additional_grad_outside_vector
  ### END CODE HERE
