...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_len = int(random.randint(4, int(self.block_size * 7 / 8)))
        element = element[:truncated_len]

        mask_content_idx = int(random.randint(0, int(len(element) * 3 / 8)))
        masked_content_len = random.randint(int(truncated_len * 1 / 8), int(truncated_len * 3 / 8))

        prefix = element[:mask_content_idx]
        masked_content = element[mask_content_idx:masked_content_len + mask_content_idx]
        suffix = element[masked_content_len + mask_content_idx:]
        ...
        x = torch.tensor([self.stoi[a] for a in input], dtype=torch.long)
        y = torch.tensor([self.stoi[b] for b in output], dtype=torch.long)
